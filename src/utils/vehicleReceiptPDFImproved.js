...
/**
 * Generates a vehicle exit receipt in PDF format with improved layout.
 * All assets (fonts, images) are defined and pre-loaded at the top to ensure
 * the process doesn't crash if a file is missing.
 */
async function generateVehicleReceiptPDFImproved({ operator, paymentMethod, plate, amountReceived, discountValue, changeGiven, finalPrice, originalAmount, category, entryTime, exitTime }) {
  ...
  const entryFormatted = formatDateTime(entryTime);
  const exitFormatted = formatDateTime(exitTime);
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.text(`PLACA: ${plate.toUpperCase()}`, leftX, doc.y, { width: printWidth, align: 'center' });
      doc.moveDown(0.8);

      // Define a reusable function for info rows (mesmo estilo dos valores)
      ...
      // Informações do veículo e operador
      addInfoRow('Operador:', operator);
      addInfoRow('Pagamento:', paymentMethod);
      addInfoRow('Categoria:', category ? category.toUpperCase() : 'N/A');
      addInfoRow('Entrada:', `${entryFormatted.date} às ${entryFormatted.time}`);
      addInfoRow('Saída:', `${exitFormatted.date} às ${exitFormatted.time}`);
      doc.moveDown(0.5);
...
module.exports = { generateVehicleReceiptPDFImproved };
